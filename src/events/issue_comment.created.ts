...
`.trim(),
      ...
      const actor = {
        name: metadata.owner === "daeuniverse" ? "dae-prow-robot" : "juicity-infra-svc",
        email:
      ...
      let msg = `🛸 Auto release process for ${metadata.repo} begins! Changelogs and release notes are generated by @${actor.name} automatically. Ref: issue [#${metadata.issue.number}: ${metadata.issue.title}](${metadata.issue.html_url})`;
