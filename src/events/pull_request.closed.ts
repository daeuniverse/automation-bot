      ...
      async (span: Span) => {
        try {
          ...
          await tracer.startActiveSpan(
            ...
            { attributes: { functionality: "audit event" } },
            async (span: Span) => {
              const msg = `🌌 ${metadata.repo} passed the release gate in PR [#${metadata.pull_request.number}: ${metadata.pull_request.title}](${metadata.pull_request.html_url}); created and pushed a new release tag ${tag}; release build is now kicked off! just chill, we are getting there 💪; workflow run: ${workflowRunUrl}.`;
              app.log.info(msg);
              span.addEvent(msg);
              await extension.tg.sendMsg(msg, [
                TELEGRAM_DAEUNIVERSE_AUDIT_GROUP_ID!,
